...
class Game {
	...
	update() {
		...
		if (this.scoreCounter >= Config.SCORE_TO_WIN) {
			...
			VariablesFromHTML.gameOverBtn.addEventListener('click', () =>
				location.reload()
			);
	...
	isOnTheSamePosition(coinPos) {
		return (
			coinPos.x === this.character.charPosition.x &&
			coinPos.y === this.character.charPosition.y
		);
	}
	...
	updateCoin() {
		...
		if (checkPosition) {
			...
			if (
				speedGame > this.gameSpeedDiffToHtml + 1 &&
				speedGame <= this.gameSpeedDiffToHtml + 2
			) {
				coinsBonus = Config.ADD_COINS_BONUS[1];
				this.scoreCounter += coinsBonus;
			} else if (
				speedGame > this.gameSpeedDiffToHtml + 2 &&
				speedGame <= this.gameSpeedDiffToHtml + 3
			) {
				coinsBonus = Config.ADD_COINS_BONUS[2];
				this.scoreCounter += coinsBonus;
			} else if (
				speedGame > this.gameSpeedDiffToHtml + 3 &&
				speedGame <= this.gameSpeedDiffToHtml + 4
			) {
	...
	gameOver() {
		return (
			this.character.charPosition.x < 1 ||
			this.character.charPosition.y < 1 ||
			this.character.charPosition.x > Config.GRID_SIZE ||
			this.character.charPosition.y > Config.GRID_SIZE
		);
	}
